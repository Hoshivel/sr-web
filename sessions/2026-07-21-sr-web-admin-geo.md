...
### 已完成（精簡摘要）
- [x] **Phase F：pull ShatteredRealms → 四立繪全接入**。二次 fetch origin/main 取得 `6ce10d0`（四角色立繪／頭像**去假棋盤格背景**、真透明 1086×1448 3:4）。由原圖以 sharp 重生 `public/art/portraits/{hakuto,shadow,sekien,aoiro}.webp`（640w、q74、~148–182KB，含 hakuto 覆蓋舊圖修正假背景）；`Characters.astro` 四英雄卡全接立繪（object-fit contain 恰滿 3:4）。四立繪已逐一目視確認角色正確、透明乾淨。build 綠、四圖落 dist、佔位 0。
